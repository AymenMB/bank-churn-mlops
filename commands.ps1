...
function Start-Container {
    Write-Host "▶️ Starting container..." -ForegroundColor Cyan
...
function Stop-Container {
    Write-Host "⏹️ Stopping container..." -ForegroundColor Cyan
...
function Deploy-ToAzure {
    Write-Host "🚀 Déploiement sur Azure Container Apps" -ForegroundColor Cyan
    ...
    try {
        ...
        Write-Host "❌ Docker n'est pas accessible. Démarrez Docker Desktop." -ForegroundColor Red
        return
    }
    
    # Validation nom ACR
    if ($ACR_NAME.Length -lt 5 -or $ACR_NAME.Length -gt 50) {
        Write-Host "❌ Nom ACR invalide: $ACR_NAME (doit être 5-50 caractères)" -ForegroundColor Red
        return
    }
    Write-Host "✅ Nom ACR validé: $ACR_NAME ($($ACR_NAME.Length) caractères)" -ForegroundColor Green
    
    Write-Host "`n📦 ÉTAPE 1: Installation des extensions Azure CLI" -ForegroundColor Cyan
    # Vérifier et installer containerapp
    ...
    if (-not $hasExtension) {
        ...
        Write-Host "✅ Extension containerapp installée" -ForegroundColor Green
    } else {
        Write-Host "✅ Extension containerapp déjà installée" -ForegroundColor Green
        az extension update --name containerapp -y --only-show-errors 2>$null
    }
    
    Write-Host "`n📦 ÉTAPE 2: Enregistrement des providers Azure" -ForegroundColor Cyan
    Write-Host "Cela peut prendre 2-3 minutes..." -ForegroundColor Yellow
    az provider register --namespace Microsoft.ContainerRegistry --wait
    az provider register --namespace Microsoft.App --wait
    az provider register --namespace Microsoft.Web --wait
    az provider register --namespace Microsoft.OperationalInsights --wait
    Write-Host "✅ Providers enregistrés" -ForegroundColor Green
    
    Write-Host "`n📦 ÉTAPE 3: Création du Resource Group" -ForegroundColor Cyan
    az group create --name $RESOURCE_GROUP --location $LOCATION --output none 2>$null
    Write-Host "✅ Resource Group: $RESOURCE_GROUP (région: $LOCATION)" -ForegroundColor Green
    
    Write-Host "`n📦 ÉTAPE 4: Création du Container Registry (ACR)" -ForegroundColor Cyan
    $acrExists = az acr show --name $ACR_NAME --resource-group $RESOURCE_GROUP 2>$null
    if (-not $acrExists) {
        Write-Host "Création de l'ACR en $LOCATION..." -ForegroundColor Yellow
        ...
        if (-not $acrResult) {
            Write-Host "⚠️ Échec en $LOCATION, tentative en $FALLBACK_LOCATION..." -ForegroundColor Yellow
        ...
        Start-Sleep -Seconds 5
        Write-Host "✅ ACR créé: $ACR_NAME (région: $LOCATION)" -ForegroundColor Green
    } else {
        Write-Host "✅ ACR existe déjà: $ACR_NAME" -ForegroundColor Green
    }
    
    Write-Host "`n📦 ÉTAPE 5: Login ACR et Push de l'image" -ForegroundColor Cyan
    ...
    if (-not $localImage) {
        Write-Host "Image locale non trouvée, build en cours..." -ForegroundColor Yellow
    ...
    Write-Host "✅ Images pushées dans ACR" -ForegroundColor Green
    
    Write-Host "`n📦 ÉTAPE 6: Création du Log Analytics Workspace" -ForegroundColor Cyan
    $LAW_NAME = "law-mlops-$USERNAME-$(Get-Random -Maximum 99999)"
    Write-Host "Création: $LAW_NAME" -ForegroundColor Yellow
    ...
    Write-Host "✅ Log Analytics créé" -ForegroundColor Green
    
    Write-Host "`n📦 ÉTAPE 7: Création du Container Apps Environment" -ForegroundColor Cyan
    $envExists = az containerapp env show `
        --name $CONTAINERAPPS_ENV `
        --resource-group $RESOURCE_GROUP 2>$null
    
    if (-not $envExists) {
        Write-Host "Création de l'environnement (cela prend 2-3 minutes)..." -ForegroundColor Yellow
        ...
        Write-Host "✅ Environment créé: $CONTAINERAPPS_ENV" -ForegroundColor Green
    } else {
        Write-Host "✅ Environment existe déjà: $CONTAINERAPPS_ENV" -ForegroundColor Green
    }
    
    Write-Host "`n📦 ÉTAPE 8: Déploiement du Container App" -ForegroundColor Cyan
    $appExists = az containerapp show `
        --name $CONTAINER_APP_NAME `
        --resource-group $RESOURCE_GROUP 2>$null
    
    if ($appExists) {
        Write-Host "Mise à jour de l'application..." -ForegroundColor Yellow
        ...
        Write-Host "Création de l'application (cela prend 2-3 minutes)..." -ForegroundColor Yellow
    ...
    Write-Host "✅ Container App déployé: $CONTAINER_APP_NAME" -ForegroundColor Green
    
    Write-Host "`n📦 ÉTAPE 9: Récupération de l'URL" -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "🗑️ Pour supprimer toutes les ressources:" -ForegroundColor Cyan
    Write-Host "  az group delete --name $RESOURCE_GROUP --yes --no-wait" -ForegroundColor White
    Write-Host ""
    Write-Host "💰 Coût estimé: 3-6€/mois" -ForegroundColor Yellow
    Write-Host "=" -NoNewline; 1..60 | ForEach-Object { Write-Host "=" -NoNewline }; Write-Host ""
    
    # Sauvegarder l'URL pour les tests
    $APP_URL | Out-File -FilePath "azure_url.txt" -Encoding utf8
    Write-Host "✅ URL sauvegardée dans azure_url.txt" -ForegroundColor Green
}
...
function Test-AzureAPI {
    ...
    if (-not (Test-Path "azure_url.txt")) {
        Write-Host "❌ Fichier azure_url.txt non trouvé. Exécutez Deploy-ToAzure d'abord." -ForegroundColor Red
        return
    }
    
    $APP_URL = (Get-Content "azure_url.txt").Trim()
    Write-Host "URL testée: https://$APP_URL" -ForegroundColor Yellow
    
    Write-Host "`n1️⃣ Test Health Check..." -ForegroundColor Cyan
    try {
        ...
        Write-Host "❌ Échec health check: $_" -ForegroundColor Red
    }
    
    Write-Host "`n2️⃣ Test Prédiction..." -ForegroundColor Cyan
    $testData = @{
        CreditScore = 650
        Age = 35
        Tenure = 5
    ...
    try {
        ...
        Write-Host "✅ Prédiction: $($prediction | ConvertTo-Json)" -ForegroundColor Green
        ...
        Write-Host "❌ Échec prédiction: $_" -ForegroundColor Red
    }
    
    Write-Host "`n3️⃣ URLs utiles:" -ForegroundColor Cyan
...
function Get-AzureLogs {
    ...
    Write-Host "📋 Récupération des logs Azure..." -ForegroundColor Cyan
    $RESOURCE_GROUP = "rg-mlops-bank-churn"
    $CONTAINER_APP_NAME = "bank-churn"
...
function Remove-AzureResources {
    Write-Host "🗑️ Suppression des ressources Azure" -ForegroundColor Yellow
    Write-Host "⚠️ ATTENTION: Cela supprimera TOUTES les ressources du groupe!" -ForegroundColor Red
    ...
    Write-Host "✅ Suppression lancée (asynchrone)" -ForegroundColor Green
    Write-Host "Vérifiez le portail Azure dans quelques minutes." -ForegroundColor Yellow
}
